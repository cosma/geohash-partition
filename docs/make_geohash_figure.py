...
def main() -> None:
    ...
    parts.append(
        f'<text x="{left_x}" y="44" font-family="{SANS}" font-size="17" font-weight="600" fill="{INK}">'
        f'<tspan font-family="{MONO}">{parent}</tspan> · precision {FINE - 1}</text>'
    )
    parts.append(
        f'<text x="{left_x}" y="68" font-family="{SANS}" font-size="13" fill="{MUTED}">'
    ...
    for char in BASE32:
        ...
        parts.append(
            f'<rect x="{x:.1f}" y="{y:.1f}" width="{w:.1f}" height="{h:.1f}" fill="{fill}" stroke="{stroke}" stroke-width="{width}"/>'
        )
        parts.append(
            f'<text x="{x + w / 2:.1f}" y="{y + h / 2 + 5:.1f}" text-anchor="middle" font-family="{MONO}" '
            f'font-size="14" font-weight="{700 if is_cell else 400}" fill="{ACCENT if is_cell else INK}">{char}</text>'
        )
    parts.append(
        f'<rect x="{left_x:.1f}" y="{top:.1f}" width="{left_w:.1f}" height="336" fill="none" stroke="{INK}" stroke-width="1.5"/>'
    )
    parts.append(
        f'<text x="{left_x}" y="{top + 336 + 24:.1f}" font-family="{SANS}" font-size="12" fill="{MUTED}">'
    ...
    _, nw_min_lon, nw_max_lat, _ = bbox(rows[0][0])
    ...
    parts.append(
        f'<text x="{right_x:.1f}" y="44" font-family="{SANS}" font-size="17" font-weight="600" fill="{INK}">'
        f'<tspan font-family="{MONO}">{cell}</tspan> and its 8 neighbours</text>'
    )
    parts.append(
        f'<text x="{right_x:.1f}" y="68" font-family="{SANS}" font-size="13" fill="{MUTED}">'
    ...
    for row in rows:
        for code in row:
            ...
            label = (
                f'<tspan fill="{MUTED}">{code[:shared]}</tspan>'
                f'<tspan fill="{ACCENT if is_cell else INK}" font-weight="700">{code[shared:]}</tspan>'
            )
            parts.append(
                f'<text x="{x + w / 2:.1f}" y="{y + h / 2 + 5:.1f}" text-anchor="middle" font-family="{MONO}" font-size="15">{label}</text>'
            )
    parts.append(
        f'<rect x="{right_x:.1f}" y="{top:.1f}" width="{right_w:.1f}" height="336" fill="none" stroke="{INK}" stroke-width="1.5"/>'
    )

    px, py = project_right(LAT, LON)
    parts.append(f'<circle cx="{px:.1f}" cy="{py:.1f}" r="5" fill="{INK}" stroke="{CARD}" stroke-width="2"/>')
    parts.append(
        f'<text x="{px + 10:.1f}" y="{py + 18:.1f}" font-family="{MONO}" font-size="12" fill="{INK}" '
    ...
    parts.append(
        f'<text x="{right_x:.1f}" y="{top + 336 + 24:.1f}" font-family="{SANS}" font-size="12" fill="{MUTED}">'
